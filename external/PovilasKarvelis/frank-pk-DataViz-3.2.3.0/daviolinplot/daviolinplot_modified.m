function h = daviolinplot_modified(AxesHandle, app, Y, varargin)
...
%
% 08/19/2024 Makoto Miyakoshi. AxesHandle supported. Accepts app object.
% 05/05/2020 Povilas Karvelis. Created.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
addOptional(p, 'colors', get(app.UIAxes, 'ColorOrder'));
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        % plot the violin
        if strcmp(confs.violin,'full')            
            h.ds(k,g) = fill(AxesHandle, [f,-fliplr(f)]+gpos(g,k),...
                [xi,fliplr(xi)],confs.colors(g,:));            
        elseif strcmp(confs.violin,'half')   
            if confs.box==3
                h.ds(k,g) = fill(AxesHandle, 1.3.*f+gpos(g,k),xi,confs.colors(g,:));
            else
                h.ds(k,g) = fill(AxesHandle, f+gpos(g,k),xi,confs.colors(g,:));
            end 
        end       
        ...
        if confs.box~=0
            ...
            % draw boxes
            if strcmp(confs.boxcolors,'same')
                h.bx(k,g) = fill(AxesHandle, [Xx(:,1)' Xx(1,:) Xx(:,2)' Xx(2,[2,1])],...
            ...
            % draw the median
            h.md(k,g) = line(AxesHandle, Xx(1,:), box_medcor(win_k),...
            ...
            if confs.whiskers==1
                ...
                % plot whiskers
                h.wh(k,g,:) = plot(AxesHandle, whi_xcor(:,k),whi_ycor(:,1,k),'k-',... 
        ...
        if confs.outliers==1    
            ...
            h.ot(k,g) = scatter(AxesHandle, xdata(ox),data_vals(ox),confs.scattersize,...
                confs.outsymbol);    
        end            
        ...
        if confs.scatter~=0 
            if strcmp(confs.scattercolors,'same')
                h.sc(k,g) = scatter(AxesHandle, xdata(~ox),data_vals(~ox),...
                    confs.scattersize,...
    ...
    if confs.linkline==1
       h.ln(g) = line(AxesHandle, gpos(g,:),pt(4,:),'color',confs.colors(g,:),...
    ...
    if confs.withinlines==1
        for s = 1:size(scdata{g},1)                
            h.wl(g) = plot(AxesHandle, squeeze(scdata{g}(s,1,:)),...
...
set(AxesHandle,'XTick',cpos,'XTickLabels',cpos,'box','off');
if ~isempty(confs.xtlabels)
    set(AxesHandle,'XTickLabels',confs.xtlabels,'XTick',cpos);
end  

% adjust x-axis margins
xlim(AxesHandle, [gpos(1)-3*box_width, gpos(end)+3*box_width]);
